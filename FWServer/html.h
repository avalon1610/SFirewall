...
"function Submit(action,type){"
	"if (check_IP()&&check_Port()){"
		...
		"var data=(type==3)?'':'{\"entrys\":['+JSON.stringify(obj)+']}';"
...
"<input type=\"button\" value=\"Show Rules\" onclick=\"Submit('/rule',3)\"/>"
...
"{"
	...
	"{"
		"return;"
	"}"
	"var my_name=$(\"input[name='input_my_name']\").val();"
	"var status=$(\"input[name='input_status']\").val();"
	"var ip=$(\"input[name='input_server_ip']\").val();"
	"if (my_name=='')"
		"my_name = Math.random().toString(36).substr(8);"
	"var data='input_my_name='+my_name+'&input_server_ip='+ip+'&input_status='+status;"
