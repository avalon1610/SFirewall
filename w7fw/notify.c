...
COMMAND_TYPE MonitorNfGetCommandType(IN BYTE*    stream,
...
__forceinline
PVOID MonitorNfpFindCharacters(IN const char *stream,
							   IN SIZE_T streamLength,
...
NTSTATUS MonitorNfParseMessageOutbound(IN BYTE *stream,
									   IN SIZE_T streamLength,
									   IN USHORT localPort,
									   IN USHORT remotePort)
{
	...
	{
		COMMAND_TYPE cmdType;
		BYTE stackSteam[1024];
		BYTE *tmpStream = NULL;
		BOOLEAN allocatedStream = FALSE;

		if (streamLength >= sizeof(stackSteam))
		{
		...
		case CMD_TYPE_CLIENT_SERVER:
			{
				...
				DoTraceMessage(TRACE_CLIENT_SERVER,
					...
					localPort,remotePort,(const char *)tmpStream);
				break;
			}
...
NTSTATUS MonitorNfParseMessageOutboundHttpHeader(IN BYTE *stream,
												 IN SIZE_T streamLength,
												 IN USHORT localPort,
												 IN USHORT remotePort)
{
	...
	if (msgStart && (bytesLeft > 0))
	{
		...
		status = MonitorNfParseMessageOutbound(msgStart,msgLength,localPort,remotePort);
	}
...
__forceinline
BOOLEAN MonitorNfpIsPrintableCharacter(IN BYTE ch)
{
	if (ch == '\r' || ch == '\n' || (ch >= 0x20 && ch <= 0x7e))
		return TRUE;
	else
...
NTSTATUS MonitorNfParseMessageInbound(IN BYTE *stream,
									  IN SIZE_T streamLength,
									  IN USHORT localPort,
									  IN USHORT remotePort)
{
	...
	if (streamLength != 0)
	{
		...
		case CMD_TYPE_CLIENT_SERVER:
			{
				RtlZeroMemory(tmpStream,stream,streamLength);
				DoTraceMessage(TRACE_CLIENT_SERVER,
...
__forceinline
BOOLEAN MonitorNfContainsNonPrintableCharacters(IN BYTE *buffer,
...
NTSTATUS MonitorNfParseMessageInboundHttpHeader(IN BYTE *stream,
												IN SIZE_T streamLength,
												IN USHORT localPort,
												IN USHORT remotePort)
{
...
NTSTATUS MonitorNfParseStreamAndTraceMessage(IN BYTE *stream,
											 IN SIZE_T streamLength,
											 IN BOOLEAN inbound,
											 IN USHORT localPort,
											 IN USHORT remotePort)
								...
								IN USHORT remotePort)
{
	...
	FwpsCopyStreamDataToBuffer(streamBuffer,stream,streamLength,&bytesCopied);
