...
--*/
{
    ...
    do
    {
		...
		NdisAllocateBufferPool(Status,&pAdapt->SendBufferPoolHandle,MAX_PACKET_POOL_SIZE);
		...
		NdisAllocateBufferPool(Status,&pAdapt->RecvBufferPoolHandle,MAX_PACKET_POOL_SIZE);
...
--*/
{
    ...
    {
		PRECV_RSVD recv_rsd = (PRECV_RSVD)(Packet->ProtocolReserved);
		PNDIS_PACKET origPkt = recv_rsd->OriginalPkt;
		if (origPkt == NULL)
		{
			PNDIS_BUFFER pOriginalBuffer = RCV_PKT_TO_ORIGINAL_BUFFER(Packet);
			PNDIS_BUFFER pPartialBuffer;
			...
			if (pOriginalBuffer != NULL)
			{
				...
				DEBUGP(DL_LOUD,("TransferComp: Pkt %p, OrigBuf %p, PartialBuf %p\n",
					Packet,pOriginalBuffer,pPartialBuffer));
			...
			NdisQueryBuffer(pOriginalBuffer,(PVOID *)&pDataBuffer,&length);
...
--*/
{
    ...
    else do
    {
        ...
        else
        {
            ...
            // 
			PUCHAR pDataBuffer;
			ULONG totalSize = HeaderBufferSize + PacketSize;
			MyPacket = PMgrAllocRecvPkt(pAdapt,totalSize,&pDataBuffer);
			if (MyPacket == NULL)
			{
				Status = NDIS_STATUS_RESOURCES;
			}
			...
			{
				NdisMoveMemory(pDataBuffer,HeaderBuffer,HeaderBufferSize);
				if (PacketSize <= LookAheadBufferSize)
				{
					PacketStatus status;
					NdisMoveMemory(pDataBuffer + HeaderBufferSize,LookAheadBuffer,PacketSize);
					...
					if (status == PacketPass)
					{
						if (pAdapt->MiniportHandle != NULL)
							NdisMIndicateReceivePacket(pAdapt->MiniportHandle,&MyPacket,1);
						PMgrFreeRecvPkt(pAdapt,MyPacket);
				...
				{
					PNDIS_BUFFER pPartialNdisBuffer;
					PNDIS_BUFFER pOriginalNdisBuffer;
					...
					NdisAllocateBuffer(&Status,
									  ...
									  pDataBuffer + HeaderBufferSize,
									  PacketSize);
					if (Status == NDIS_STATUS_SUCCESS)
					{
						PRECV_RSVD recv_rsd = (PRECV_RSVD)(MyPacket->ProtocolReserved);
						NdisUnchainBufferAtFront(MyPacket,&pOriginalNdisBuffer);
						RCV_PKT_TO_ORIGINAL_BUFFER(MyPacket) = pOriginalNdisBuffer;

						recv_rsd->OriginalPkt = NULL;
						//Link in the partial buffer for NdisTransferData to operate on
						NdisChainBufferAtBack(MyPacket,pPartialNdisBuffer);
						DEBUGP(DL_LOUD,("Receive:setting up for TransferData:Pkt %p,OriginalBuf %p,PartialBuf %p\n",
							MyPacket,pOriginalNdisBuffer,pPartialNdisBuffer));
						...
						if (Status == NDIS_STATUS_SUCCESS)
						{
							PtTransferDataComplete(pAdapt,MyPacket,Status,BytesTransferred);
						}
						return Status;
					}
				}
